...
- **Open-Meteo** — консенсус-прогноз по нескольким численным моделям (ECMWF, GFS,
  ICON, MET Nordic). Вызов через `open-meteo-proxy` (отдельный контейнер, SOCKS5
  прокси для обхода блокировок). Результат — `ForecastConsensus` с разбросом
  температур и уверенностью. **Рендерится SSR** с таймаутом 3 с — если прокси
  тормозит, консенсус пропускается без задержки страницы.
...
моделями (ECMWF, GFS, ICON, MET Nordic) и уровень уверенности. LLM упоминает
надёжность прогноза в сводке — пользователь видит, насколько модели согласны.

### Консенсус Open-Meteo

Свод прогноза по 4 численным моделям (ECMWF IFS, GFS, ICON, MET Nordic) —
дополнительный сигнал надёжности поверх основного прогноза MET. Показывает
разброс температур между моделями и уровень уверенности (high/medium/low).

**Архитектура:**

```
51pogoda (Next) → open-meteo-proxy (Node, :3001) → SOCKS5 → api.open-meteo.com
                     ↑ in-memory кэш 30 мин
```

- **`open-meteo-proxy`** — отдельный контейнер (`C:\work\open-meteo-proxy`),
  Node.js HTTP-сервер. Проксирует запросы к `api.open-meteo.com` через SOCKS5
  (т.к. Open-Meteo на Hetzner заблокирован в РФ). Имеет **in-memory кэш 30 мин**
  по ключу `lat,lon,params` — повторные запросы отдаются мгновенно без SOCKS.
  Заголовок `X-Cache: HIT/MISS` для отладки.
- **`fetchOpenMeteo`** (`entities/weather/api/open-meteo-client.ts`) — клиент
  в 51pogoda. Обращается к `OPEN_METEO_PROXY` (по умолч.
  `http://127.0.0.1:3001/forecast`). Результат кэшируется в **Next Data Cache
  6 часов** (`CONSENSUS_REVALIDATE`, теги `weather`/`weather:<slug>`).
- **`getCityConsensus`** — без таймаута, 10с upstream timeout. Используется
  в API-роуте `/api/consensus/[slug]` (клиентский виджет).
- **`getCityConsensusTimed`** — таймаут 3с. Используется в SSR (рендер страницы)
  — если прокси не ответил за 3с, консенсус пропускается, страница не ждёт.
- **`buildConsensus`** — чистая функция, сводит данные 4 моделей в
  `ForecastConsensus` (дни с разбросом, уверенностью, усреднённой температурой).

**Частота запросов к Open-Meteo:**

| Сценарий | Сетевых запросов/день |
|---|---|
| Города (16, ISR 30мин) | 16 × 4 = ~64 |
| Не-города (посещённые) | ~10–30 × 4 = ~40–120 |
| Крон 8:00 | 0–4 (почти все из Data Cache) |
| **Итого** | **~64–200** |

Все остальные вызовы (ISR, посетители, клиентский виджет) берут данные из
Next Data Cache мгновенно — без обращения к прокси.

**Деградация:** консенсус — доп. сигнал, страница работает без него. При
недоступности прокси: SSR пропускает за 3с, клиентский виджет получает 502
и скрывается. ИИ-сводка генерируется без данных о надёжности.

**API:** `/api/consensus/[slug]` — `Cache-Control: s-maxage=21600,
stale-while-revalidate=43200` (CDN кэш 6ч + 12ч stale).
