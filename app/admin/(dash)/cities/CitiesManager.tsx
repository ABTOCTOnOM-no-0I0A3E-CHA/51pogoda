...
export function CitiesManager({ initialCustom }: { initialCustom: City[] }) {
  ...
  async function submit() {
    ...
    try {
      ...
      setStatus({ kind: "ok", text: `Добавлено: ${data.city?.name} (/${data.city?.slug})` });
      setForm({ ...EMPTY });
      void loadCustom();
    } catch (e) {
  ...
  return (
    <>
      <div className={styles.card}>
        <div className={styles.cardTitle}>Новая точка</div>
        <div className={styles.row}>
          <div className={styles.field}>
            <label className={styles.label}>slug</label>
            <input className={styles.input} value={form.slug} onChange={(e) => set("slug", e.target.value)} placeholder="my-point" />
          </div>
          <div className={styles.field}>
        ...
        <div className={styles.btnRow}>
          <button className={`${styles.btn} ${styles.btnPrimary}`} disabled={busy} onClick={submit}>
            {busy ? "Добавление…" : "Добавить точку"}
          </button>
        </div>
        {status && (
